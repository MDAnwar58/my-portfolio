<x-layouts.app :title="__('Add Skill')">
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item>Add Skill</flux:breadcrumbs.item>
    </flux:breadcrumbs>

    <div class="mt-6">
        <div class=" mx-auto">
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-6">Add New Skill</h2>

                <form method="POST" action="{{ route('admin.skills.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-4">
                        <x-input type="text" name="name" id="name" placeholder="Enter skill name" label="Skill Name" :value="old('name')" required />

                        <x-input type="date" name="date_of_exp" id="date_of_exp" placeholder="Enter date of experience" label="Date of Experience (Optional)" :value="old('date_of_exp')" />

                        <x-input type="url" name="url" id="url" placeholder="Enter URL" label="URL (Optional)" :value="old('url')" />

                        <x-input type="file" name="image_url" id="image_url" label="Skill Image" />
                    </div>
                    ...
                    <div class="flex items-center justify-end mt-6 gap-3">
                        <button type="submit" class="flex items-center px-4 py-2 text-sm font-medium text-white bg-black rounded-lg hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-800/75 transition-colors duration-200 ease-in-out" id="submit-button">
                            <span id="button-text">Save Skill</span>
