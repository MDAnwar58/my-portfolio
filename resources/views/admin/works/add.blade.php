<x-layouts.app :title="__('Add Work')">
    <div class="flex justify-between items-center mb-6">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Add Work</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Add New Work</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Fill in the details below to add a new work to your portfolio.</p>
        </div>

        <form method="POST" action="{{ route('admin.work.store') }}" class="px-6 py-6" enctype="multipart/form-data">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <x-input type="text" name="name" id="name" placeholder="Enter work name" label="Work Name" />
            ...
            <x-textarea name="short_desc" id="short_desc" placeholder="Briefly describe this work..." label="Short Description" rows="3" />

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 mt-6">
                <x-input type="url" name="live_demo_link" id="live_demo_link" placeholder="https://example.com" label="Live Demo Link" />

                <x-input type="url" name="github_link" id="github_link" placeholder="https://github.com/username/repo" label="GitHub Link" />
            </div>
            ...
            <x-textarea name="more_info" id="more_info" placeholder="Provide additional details about this work..." label="More Information" rows="10" />
                    ...
                    $('#more_info').summernote({
                        ...
                        , callbacks: {
                            onInit: function() {
                                ...
                                const initialValue = $('#more_info').val();
                                if (initialValue) {
                                    $('#more_info').summernote('code', initialValue);
                                }
                            }
                        }
                    });
